  ...
  return (
    <div className="fines">
      ...
      {!displayLoading && !displayError && filteredFines.length === 0 && (
        <EmptyState
          title={filter === 'all' ? 'No fines found' : `No ${filter} fines`}
          message={filter === 'all' ? 'You currently have no traffic fines on your account.' : 'Try another status filter.'}
        />
      )}
      ...
      <div className="fines-list">
        {filteredFines.map(fine => (
          <div key={fine._id || fine.id}>
            <FineCard fine={fine} />
            <div className="fine-status-label">
