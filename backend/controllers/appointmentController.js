...
exports.createAppointment = async (req, res) => {
  try {
    ...
    const appt = await Appointment.create({ userId: req.user.id, type, date, time, testingCenter });
...
exports.allAppointments = async (req, res) => {
  try {
    ...
    const filter = {};
    if (req.query.status) {
      if (!allowedStatuses.includes(req.query.status)) return res.status(400).json({ message: "Invalid appointment status" });
      filter.status = req.query.status;
    }
    if (req.query.type) {
      if (!["learner", "drivers"].includes(req.query.type)) return res.status(400).json({ message: "Invalid appointment type" });
